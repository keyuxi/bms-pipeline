...
# Within-sample per-cell QC done
# Secondary filtering not done yet, e.g. is_neuron 
# Inputs (CLI):
...
mk_obj_from_dir <- function(dir_path, sample_id) {
    ...
    obj <- subset(obj, subset = nCount_RNA > 200)
    ...
    kept_dims <- seq_len(opt$anchor_dims)
    anchors  <- FindTransferAnchors(reference=allen, query=obj, dims=kept_dims, reference.reduction="pca")

    # Loop over cluster, subclass, neighborhood, class
    preds_df <- NULL
    for (lab in label.cols) {
        ...
        p <- p[, c(1, ncol(p))]
        colnames(p) <- c(lab, paste0(lab, ".prediction.score.max"))
...
kept_dims <- seq_len(opt$umap_dims)
gex <- RunUMAP(gex, dims=kept_dims, seed.use=42, verbose=FALSE)
gex <- FindNeighbors(gex, dims=kept_dims)
gex <- FindClusters(gex, resolution=opt$cluster_res)

# ---- outputs -----------------------------------------------------------------
saveRDS(gex, opt$out_rds)

# ---- filter by seurat cluster ---------------------------------------
md <- gex@meta.data
